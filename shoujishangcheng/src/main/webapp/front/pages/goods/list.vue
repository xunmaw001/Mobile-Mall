	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
		...
		<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			...
			<view v-if="queryIndex==1" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.goodsPinpai" type="text" placeholder="品牌"></input>
			</view>
			<view class="action">
		...
		<scroll-view scroll-x="true" class="tabView"
			:style='{"padding":"8rpx 0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"rgba(0,0,0,1)","backgroundColor":"#e2e2e2","borderRadius":"0","borderWidth":"0","width":"100%","borderStyle":"solid","height":"auto"}'>
			<view class="tab" :class="categoryName===0?'tabActive':''" @tap="categoryClick(0)">
				全部</view>
		...
		<view class="uni-product-list" :style='{"borderRadius":0,"backgroundColor":"#efefef"}'>
			<view @tap="onDetailTap(product)" class="uni-product"
				...
				<view class="uni-product-price"
					:style='{"fontSize":"24rpx","lineHeight":"48rpx","color":"#999","textAlign":"left"}'>
		...
		<button v-if="isAuth('goods','新增')" class="add-btn" @click="onAddTap()">新增</button>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.goodsPinpai = "";
				this.searchForm.goodsBiaoqianTypes = "";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				debugger
				let res = await this.$api.page(`goods`, params);
				// 如果是第一页数据置空
			...
			async search() {
				...
				if (this.searchForm.goodsPinpai) {
					searchForm['goodsPinpai'] = this.searchForm.goodsPinpai 
				}
				if (this.searchForm.goodsBiaoqianTypes) {
					searchForm['goodsBiaoqianTypes'] = this.searchForm.goodsBiaoqianTypes 
				}
				
				
				let res = await this.$api.page(`goods`, searchForm);
				// 如果是第一页数据置空
