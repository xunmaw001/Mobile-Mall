		...
		<view class="container">
			...
			<view class="detail-content">
				...
				<view class="price detail-list-item priceFavor"
					...
					<view class="text" :style='{"color":"#333","textAlign":"left","fontSize":"28rpx"}'>¥{{detail.goodsNewMoney}}
					</view>
					<view class="text icon"
						:style='{"color":"rgba(255, 0, 0, 1)","textAlign":"right","fontSize":"40rpx"}'>
						<view class="cuIcon-favorfill" @click="shoucang"></view>
					</view>
				</view>
				...
				<view class="detail-list-item"
					...
					<view class="lable"
						:style='{"width":"auto","fontSize":"28rpx","color":"rgba(51, 153, 153, 1)","textAlign":"left"}'>
						品牌：</view>
					<view class="text" :style='{"color":"#333","textAlign":"left","fontSize":"28rpx"}'>{{detail.goodsPinpai}}
					</view>
				</view>
				<view class="detail-list-item"
					:style='{"boxShadow":"0 0 10rpx rgba(0,0,0,.3)","margin":"0 0 10rpx 0","borderColor":"rgba(51, 153, 153, 1)","backgroundColor":"#fff","borderRadius":"10rpx","borderWidth":"2rpx","borderStyle":"solid","height":"68rpx"}'>
					<view class="lable"
						:style='{"width":"auto","fontSize":"28rpx","color":"rgba(51, 153, 153, 1)","textAlign":"left"}'>
						规格：</view>
					<view class="text" :style='{"color":"#333","textAlign":"left","fontSize":"28rpx"}'>{{detail.goodsGuige}}
					</view>
				</view>
			...
			<view class="time-content"
				...
				<view class="content" :style='{"color":"#333","fontSize":"28rpx"}'>
					<rich-text style="font-size: 33upx;line-height: 50upx;letter-spacing: 5upx;"
						:nodes="detail.goodsContent"></rich-text>
				</view>
			</view>

			<view class="time-content"
				:style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.3)","margin":"0 0 20rpx 0","borderColor":"rgba(102, 204, 204, 1)","backgroundColor":"#fff","borderRadius":"10rpx","borderWidth":"2rpx","borderStyle":"solid"}'>
				<view class="comoment-header"
					:style='{"borderColor":"rgba(255, 4, 4, 1)","backgroundColor":"#fff","borderRadius":"0px","borderStyle":"solid","borderWidth":"0 0 2rpx 0"}'>
					<view :style='{"color":"rgba(255, 0, 0, 1)","fontSize":"28rpx"}'>
						评论
					</view>
					<view
						:style='{"borderColor":"red","backgroundColor":"#fff","color":"rgba(255, 0, 0, 1)","borderRadius":"0","borderWidth":"0","width":"auto","fontSize":"28rpx","borderStyle":"solid","isBtn":true,"height":"60rpx"}'
				...
				<view class="cu-list comment" style="margin-top: 20upx;">
					<view
						:style='{"borderColor":"rgba(255, 0, 0, 0.2)","backgroundColor":"#fff","borderRadius":"0","borderStyle":"dashed","borderWidth":"0 0 2rpx 0"}'
						v-for="(item,index) in commentList" v-bind:key="index" class="cu-item comment-item">
						<view class="content">
							<view :style='{"color":"rgba(102, 204, 204, 1)","textAlign":"left","fontSize":"28rpx"}'
								class="text-grey">{{item.yonghuName}}</view>
							<view :style='{"color":"#666","textAlign":"left","fontSize":"24rpx"}'
								class="text-gray text-content text-df content">
								{{item.goodsCommentbackContent}}
							</view>
							<view :style='{"color":"rgba(0, 29, 251, 1)","textAlign":"right","fontSize":"24rpx"}'
								class="margin-top-sm text-gray text-df">{{item.createTime}}</view>
							<view v-if="item.goodsReplyContent" :style='{"color":"#666","textAlign":"left","fontSize":"24rpx"}'
								class="text-gray text-content text-df content">
								回复:{{item.goodsReplyContent}}
							</view>
						</view>
					</view>
			...
			<view class="bottom-content cu-bar bg-white tabbar border shop">
				...
				<view style="text-align: right;">
					...
					<button @tap="onBuyTap" style="margin-right: 10upx;" class="cu-btn bg-red round shadow-blur"
						:style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[3],borderColor:btnColor[3]}">立即订购</button>
				</view>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap() {
				uni.setStorageSync('paytable', 'goods');
			...
			async shoucang() {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.save('goodsCollection', {
			...
			onAcrossTap(tableName) {
				uni.setStorageSync('crossTable', 'goods');
			...
			async init() {
				...
				//修改富文本的图片样式
				this.detail.goodsContent = this.detail.goodsContent.replace(/img src/gi,
			...
			async upCallback(mescroll) {
				let res = await this.$api.page('goodsCommentback', {
					page: mescroll.num,
					limit: mescroll.size,
					goodsId: this.id
				});
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('goods', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('goods', this.detail);
				this.$utils.msg('审核成功');
